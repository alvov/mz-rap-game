import {MAXIMUM_NEWS_READ, MAXIMUM_RECORD_TURNS} from "../../consts";
import type {RecordLoops, RecordShot} from "../../ducks/record";
...
export function generateShareHash(
  {startTimestamp, loops, news}: {| startTimestamp: number | null, loops: RecordLoops[], news: RecordShot[] |}
) {
  const loopChunks = [];
  ...
  const newsChunks = [];
  for (const {start, id} of news) {
    newsChunks.push(`${start}${addSeparator}${id}`);
  }
...
export function generatePlayList(hash: string): {| loops: RecordLoops[], news: RecordShot[] |} | null {
  try {
